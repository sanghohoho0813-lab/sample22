  ...
  return (
    <div className={`relative overflow-hidden ${ratio} ${className}`} style={{ background: `linear-gradient(${angle}deg, hsl(${hue} 45% 92%), hsl(${hue} 40% 82%))` }}>
      {showImg && (
        ...
        <img src={`/assets/${assetKey}.jpg`} alt={label ?? ""} className="absolute inset-0 h-full w-full object-cover" loading="lazy" onError={() => setFailed(true)} />
      )}
      {!showImg && (
        <div className="absolute inset-0 flex items-center justify-center">
          {variant === "product" ? (
            <div className="flex flex-col items-center gap-1 select-none">
              ...
              <span className="text-[11px] font-medium" style={{ color: `hsl(${hue} 35% 35%)` }}>{CAT_LABEL[category]}</span>
            </div>
          ) : (
            <>
              <div className="absolute inset-0 pattern-dots" />
              <span className="absolute left-3 top-3 rounded-md bg-white/70 px-2 py-0.5 text-[11px] font-semibold" style={{ color: `hsl(${hue} 35% 30%)` }}>사진 준비 중 · {assetKey ?? "photo"}</span>
            </>
          )}
        </div>
      )}
