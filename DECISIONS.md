...
## D-19 샘플 브릿지 CTA는 셸(Shell) 1곳에만 심고, 링크·문구는 `lib/brand.ts` 단일 상수로 관리
- **WHY**: "고객용 화면·AX 대시보드 둘 다" 요구를 페이지 25개에 각각 붙이면 새 페이지마다 누락되고, 링크가 바뀔 때 25곳을 고쳐야 한다. `CustomerShell`·`AxShell`의 `</main>` 뒤 1회 삽입으로 전 라우트가 자동 상속. 링크·문구는 `MIRAE_LINKS`/`BRIDGE_COPY` 한 파일에서만 수정(컴포넌트 props로도 덮어쓰기 가능).
- **WHY NOT** 페이지별 삽입: 유지보수 비용이 라우트 수에 비례. **WHY NOT** 하드코딩 URL: 샘플이 늘어날수록 수정 지점이 곱절.
- **Checkout 제외**: 결제 진행 중 외부 이탈 유도는 전환을 깎는다. 주문 흐름(`plain`)에서만 미노출.
- **색상**: 고객 화면은 브랜드 고정색, AX는 `--t-primary`/`--t-shell` Theme Token을 따라 9 테마에서 자동 재착색. `color-mix()`는 미지원 브라우저에서 그라데이션 전체가 무효가 되어 흰 글자/흰 배경 위험이 있어 `rgb(var(--cta-rgb) / a)` + `backgroundColor` 폴백으로 대체.
- **REVISIT WHEN**: 샘플이 여러 저장소로 늘어나면 `brand.ts`를 공용 패키지로 추출.
